...
def is_summer_rain_day(d):
    summer_rain = ["-07-","-08-"]
    if any([m in d['date'] for m in summer_rain]) and d['prcp'] >= 1.0:
        return True
    return False

banhodechuva = list(filter(is_summer_rain_day, weatherdata))
print(len(banhodechuva))
pprint.pp(banhodechuva)